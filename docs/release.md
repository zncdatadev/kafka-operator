...
This document describes the standard release process for kafka-operator.
...
2. **Docker image** — uses `VERSION` directly:
   `quay.io/zncdatadev/kafka-operator:<version>`
...
4. **Helm chart publish** — pushes to
   `quay.io/kubedoopcharts/kafka-operator:<version>` (OCI registry)
...
```bash
gh api repos/zncdatadev/kafka-operator/git/refs \
  -f ref=refs/heads/release-0.x \
  -f sha=$(gh api repos/zncdatadev/kafka-operator/git/ref/heads/main --jq .object.sha)
```
...
- Docker image is available at
  `quay.io/zncdatadev/kafka-operator:x.y.z-dev`
- Helm chart is available at
  `quay.io/kubedoopcharts/kafka-operator:x.y.z-dev`
...
- **Chainsaw Test** — Runs Chainsaw E2E tests across multiple Kubernetes and Kafka versions
...
- **Release Image** — Builds and pushes multi-arch Docker image using the root
  Dockerfile to `quay.io/zncdatadev/kafka-operator:<version>`, and signs the
  image with Cosign
- **Release Chart** — Publishes the Helm chart to
  `quay.io/kubedoopcharts/kafka-operator:<version>` (OCI registry) and
  updates the [kubedoop-helm-charts](https://github.com/zncdatadev/kubedoop-helm-charts)
  index

## Versioning Convention

kafka-operator follows [Semantic Versioning](https://semver.org/):
